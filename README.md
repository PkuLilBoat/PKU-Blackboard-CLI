...
如果你想把 `pku3b` 当成 AI、脚本或自动化流程的后端来调用，现在已经有一批命令支持稳定 Markdown 输出。

例如：

```bash
pku3b course --markdown list --all-term
pku3b announcement --markdown ls --all-term
pku3b announcement --markdown show <ID> --all-term
pku3b document --markdown list --all-term
pku3b document --markdown show <ID> --all-term
pku3b assignment --markdown list --all-term
pku3b video --markdown list --all-term
pku3b tree --markdown find <COURSE_ID> "Week 1"
```

如果你想了解这次 AI CLI 改造的文档入口，可以直接看：
