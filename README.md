...
如果你想把 `pku3b` 当成 AI、脚本或自动化流程的后端来调用，现在已经有一批命令支持稳定 JSON 输出。

例如：

```bash
pku3b course --json list --all-term
pku3b announcement --json ls --all-term
pku3b announcement --json show <ID> --all-term
pku3b document --json list --all-term
pku3b document --json show <ID> --all-term
pku3b assignment --json list --all-term
pku3b video --json list --all-term
pku3b tree --json find <COURSE_ID> "Week 1"
```
